...
from dacapo.basics.arraytypes import Intensities, Annotations
...
zarr_source = ZarrSource(
    ...
    ds_name="volumes/raw",
    array_type=Intensities(shift=0, scale=0),
)
...
def mk_train_gt(path):
    ...
    return ZarrSource(
        ...
        array_type=Annotations(num_classes=2),
    )


def mk_val_raw(path):
    noise = np.random.randn(2, 50, 50, 50)
    label_0 = np.zeros_like(noise)
    noise_0 = np.random.randn(2, 50, 50, 50)
    label_1 = np.zeros_like(noise)
    noise_1 = np.random.randn(2, 50, 50, 50)
    for i in range(0, 50, 10):
        label_0[i:i+5, :, :] = 1
        label_1[i+5:i+10, :, :] = 1
    data = noise + label_0 * noise_0 * 0.6 + label_1 * noise_1 * 1.9
    ...
    dataset = zarr_container.create_dataset(
        "volumes/val/raw", data=data, shape=[50, 50, 50], dtype=np.float64
    )
    dataset.attrs["axes"] = ["c", "z", "y", "x"]
    dataset.attrs["resolution"] = (1, 1, 1)
    dataset.attrs["offset"] = (0, 0, 0)
    ...
    return ZarrSource(
        name="val_raw_zarr",
        filename=path / "data.zarr",
        ds_name="volumes/val/raw",
        array_type=Intensities(shift=0, scale=0),
    )


def mk_val_gt(path):
    noise = np.zeros((50, 50, 50), dtype=np.uint64)
    label_0 = np.zeros_like(noise)
    label_1 = np.zeros_like(noise)
    for i in range(0, 50, 10):
        label_0[i:i+5, :, :] = 1
    ...
    dataset = zarr_container.create_dataset(
        "volumes/val/gt", data=data, shape=[50, 50, 50], dtype=np.uint64
    )
    dataset.attrs["axes"] = ["z", "y", "x"]
    dataset.attrs["resolution"] = (1, 1, 1)
    dataset.attrs["offset"] = (0, 0, 0)
